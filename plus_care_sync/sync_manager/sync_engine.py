...
class SyncEngine:
	...
	def _write_doc_directly(self, doctype, name, data, is_new):
		...
		if is_new:
			# SLE records are auto-generated by ERPNext submission hooks. In Full Database
			# sync, SLEs are also pulled directly from live. If the same voucher already has
			# an SLE locally (created by a local submission), skip the insert to prevent
			# duplicate ledger entries that double-count stock.
			if doctype == "Stock Ledger Entry":
				voucher_no = data.get("voucher_no")
				item_code = data.get("item_code")
				warehouse = data.get("warehouse")
				if voucher_no and item_code and warehouse:
					existing = frappe.db.exists("Stock Ledger Entry", {
			...
			# Keep naming series counter ahead of the synced document name so the
			# next locally-created document doesn't collide with it.
			self._update_naming_series_counter(name, data)
		else:
	...
	def sync_naming_series(self):
		"""Pull tabSeries counters from the remote server via the plus_care_sync
		custom endpoint (get_series_data).  Falls back gracefully if the endpoint
		is unavailable (e.g. plus_care_sync not installed on live).
		...
		try:
			endpoint = (
				f"{self.remote_url}/api/method/"
				"plus_care_sync.sync_manager.sync_engine.get_series_data"
			)
			response = requests.get(endpoint, headers=self.get_headers(), timeout=30)
			if response.status_code in (404, 417):
				self.log_sync_info(
					"tabSeries",
					"Skipped: get_series_data not available on live server "
					"(plus_care_sync may not be installed there).",
				)
				return 0
			if response.status_code != 200:
			...
			for row in rows:
				...
				if exists:
					frappe.db.sql(
						"UPDATE `tabSeries` SET current = %s WHERE name = %s", (current, name)
					)
				else:
					frappe.db.sql(
						"INSERT INTO `tabSeries` (name, current) VALUES (%s, %s)", (name, current)
					)
				total += 1
			frappe.db.commit()
			return total
	...
	def _update_naming_series_counter(self, name, data):
		"""Advance tabSeries so the just-synced document name is never re-used.

		Derives the series prefix from the naming_series field and the document
		name itself — no date-token parsing required.  Runs best-effort; any
		exception is swallowed so the main sync is never interrupted.
		"""
		naming_series = data.get("naming_series")
		if not naming_series or not name:
			return
		try:
			num_digits = len(naming_series) - len(naming_series.rstrip("#"))
			if num_digits == 0 or len(name) < num_digits:
				return
			suffix_str = name[-num_digits:]
			try:
				counter = int(suffix_str)
			except ValueError:
				return
			prefix = name[:-num_digits]
			if not prefix:
				return
